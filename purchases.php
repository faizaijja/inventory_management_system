...
<body>
        ...
        <div class="side-nav">
            ...
            <nav class="nav-links">
                ...
                <a href="purchases.php" style="text-decoration: none; color:inherit;">
                    <?php if ($_SESSION['role'] == 'admin' || $_SESSION['role'] == 'manager'): ?>
                        <div class="nav-item active" data-id="purchases">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="container">
            <div class="main-content">
                        ...
                        <div class="filter-container">
                            ...
                            <div class="filter-controls">
                                <div class="addition">
                                    ...
                                    <input type="button" id="purchaseBtn" class="add-purchase" value="Make purchase">
                                </div>
                                ...
                                <select name="category" id="categoryFilter"
                                    ...
                                    <?php
                                    $sql = "SELECT DISTINCT category FROM purchases";
                        ...
                        <table class="product-display">
                            ...
                            <tbody>
                                ...
                                if (!empty($categoryFilter) && !empty($searchTerm)) {
                                    ...
                                    $stmt = $conn->prepare("SELECT * FROM purchases WHERE category = ? AND item LIKE ? LIMIT ?, ?");
                                    ...
                                    $stmt = $conn->prepare("SELECT * FROM purchases WHERE category = ? LIMIT ?, ?");
                                    $stmt->bind_param("sii", $categoryFilter, $start_from, $limit);
                                } elseif (!empty($searchTerm)) {
                                    // Only search term provided
                                    $stmt = $conn->prepare("SELECT * FROM purchases WHERE item LIKE ? LIMIT ?, ?");
                                    $searchParam = "%$searchTerm%";
                                    $stmt->bind_param("sii", $searchParam, $start_from, $limit);
                                } else {
                                    // No filters provided
                                    $stmt = $conn->prepare("SELECT * FROM purchases LIMIT ?, ?");
                                        ...
                                        if ($_SESSION['role'] == 'admin') {
                                            echo "<td>
                                                   <div class='action-buttons'>
                                                        <button class='edit-btn' data-id='" . $row['purchase_id'] . "'>Edit</button>
                                                        <button class='delete-btn' data-id='" . $row['purchase_id'] . "'>Delete</button>
                                                    </div>
                                                </td>";
                                        }
                ...
                <div class="pagination-container">
                    ...
                    if (!empty($categoryFilter) && !empty($searchTerm)) {
                        $stmt_total = $conn->prepare("SELECT COUNT(*) FROM purchases WHERE category = ? AND item LIKE ?");
                        $searchParam = "%$searchTerm%";
                        $stmt_total->bind_param("ss", $categoryFilter, $searchParam);
                    } elseif (!empty($categoryFilter)) {
                        $stmt_total = $conn->prepare("SELECT COUNT(*) FROM purchases WHERE category = ?");
                        $stmt_total->bind_param("s", $categoryFilter);
                    } elseif (!empty($searchTerm)) {
                        $stmt_total = $conn->prepare("SELECT COUNT(*) FROM purchases WHERE item LIKE ?");
                        $searchParam = "%$searchTerm%";
                        $stmt_total->bind_param("s", $searchParam);
                    } else {
                        $stmt_total = $conn->prepare("SELECT COUNT(*) FROM purchases");
                    }
                    ...
                    if ($page > 1) {
                        echo "<a href='purchases.php?page=" . ($page - 1) . "&category=" . $categoryFilter . "&search=" . urlencode($searchTerm) . "' class='pagination-nav'>&laquo; Previous</a> ";
                    }

                    // Page numbers
                    for ($i = 1; $i <= $total_pages; $i++) {
                        $active = ($i == $page) ? "active" : "";
                        echo "<a href='purchases.php?page=" . $i . "&category=" . $categoryFilter . "&search=" . urlencode($searchTerm) . "' class='" . $active . "'>" . $i . "</a> ";
                    }

                    // Next button
                    if ($page < $total_pages) {
                        echo "<a href='purchases.php?page=" . ($page + 1) . "&category=" . $categoryFilter . "&search=" . urlencode($searchTerm) . "' class='pagination-nav'>Next &raquo;</a>";
                    } else {
    ...
    <div id="purchaseModal" class="modal">
        <div class="modal-content">
            ...
            <form id="purchaseForm" method="POST" action="process_purchases.php">
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="item" name="item" required>
                            ...
                            while ($cat_row = $cat_result->fetch_assoc()) {
                                ?>
                                <option value="<?php echo htmlspecialchars($cat_row['item']); ?>"
                                    data-price="<?php echo htmlspecialchars($cat_row['unit_price']); ?>">
                                    <?php echo htmlspecialchars($cat_row['item']); ?>
                                </option>
                            <?php } ?>
                ...
                <button type="submit" class="btn-checkout">Add Purchase</button>
            </form>
        </div>
    </div>
    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            <span class="close">&times;</span>
            <h2>Edit Purchase</h2>
            <form id="editForm" method="POST" action="update_purchase.php">
                ...
                <button type="submit" class="btn-checkout">Update Purchase</button>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const priceField = document.getElementById('price'); // Assuming you have a price input with id="price"
        ...
        function editProduct(productId) {
            ...
            window.location.href = 'edit_purchases.php?id=' + productId;
        }
        ...
        function deleteProduct(productId) {
            if (confirm('Are you sure you want to delete this product?')) {
                ...
                xhr.open('POST', 'delete_purchases.php', true);
                ...
                xhr.send('purchase_id=' + productId);
            }
        }
        document.addEventListener('DOMContentLoaded', function () {
            // Get references to the select element and the price input field
